...
from autorole.queue import CONCLUDING_Q, DEAD_LETTER_Q
...
@pytest.mark.asyncio
async def test_concluding_worker_success_calls_done_callback(repo, queue_backend, tmp_path):
...
@pytest.mark.asyncio
async def test_concluding_worker_stage_failure_routes_to_dlq(repo, queue_backend, tmp_path):
    input_fixture = load_fixture("concluding_input.json")
...
@pytest.mark.asyncio
async def test_concluding_worker_unhandled_exception_nacks(repo, queue_backend, db, tmp_path):
    input_fixture = load_fixture("concluding_input.json")
    ...
    assert await queue_row_count(db, CONCLUDING_Q) == 1
