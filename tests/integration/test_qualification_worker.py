...
@pytest.mark.asyncio
async def test_qualification_worker_duplicate_seed_is_acknowledged(repo, tmp_path):
    input_fixture = load_fixture("exploration_seed_input.json")
    output_fixture = load_fixture("packaging_input.json")
    output_fixture["tailored"]["tailoring_degree"] = 0
    ...
    worker = QualificationWorker(
        ...
        tailoring_stage=MockStage(_result(True, output_fixture)),
        repo=repo,
        logger=logging.getLogger("test.qualification.duplicate"),
        artifacts_root=tmp_path,
        config=WorkerConfig(SCORING_Q, PACKAGING_Q, DEAD_LETTER_Q, poll_interval_seconds=0),
        max_attempts=2,
    )
    ...
    first = make_worker_message(input_fixture, SCORING_Q, PACKAGING_Q)
    second = make_worker_message(input_fixture, SCORING_Q, PACKAGING_Q)
    await queue.enqueue(SCORING_Q, first)
    await queue.enqueue(SCORING_Q, second)

    pulled_first = await queue.pull(SCORING_Q)
    assert pulled_first is not None
    await worker.process(queue, pulled_first)

    pulled_second = await queue.pull(SCORING_Q)
